...
SELECT DNUM AS FOLIO, TIDESCR AS TIPO_MOV,DREFER AS FACTURA_TICKET, CATDESCR AS TIENDA,IEAN AS EAN,ICOD AS MODELO,IDESCR AS DESCRIPCION,FAM2.FAMDESCR AS DEPARTAMENTO,
...
AICANTF AS VENTA_EN_UNIDADES,AICOSTO AS COSTO,AICOSTO * 1.16  AS COSTO_C_IVA,AICOSTO * AICANTF * 1.16 AS COSTO_TOTAL,AIPRECIO AS PRECIO,
AIPRECIO * 1.16 AS PRECIO_C_IVA,ROUND((AICANTF*AIPRECIO)+(IF(IPORCIVA=0,(AICANTF*AIPRECIO)*((DPORCIVA/100)),0)),2)AS PRECIO_TOTAL,
DFECHA AS FECHA,
IF(IPORCIVA=0,'16%','EXENTO') as TIPO_IVA,
IF(DCANCELADA=0,'NO','CANCELADO') as CANCELADO,
CLICOD AS COD_CLI,
CLINOM AS CLIENTE,
DMULTICIA AS NUM_CIA
...
WHERE (DITIPMV='F' OR DITIPMV='A' OR DITIPMV='C' OR DITIPMV='FL' OR DITIPMV='FK'OR DITIPMV='FX'OR DITIPMV='FT'OR DITIPMV='G' OR  DITIPMV='TD'OR DITIPMV='TL'OR DITIPMV='TC')