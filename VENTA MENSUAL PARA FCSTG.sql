...
IF(MONTH(DFECHA)>=10,CONCAT('01','/',MONTH(DFECHA), '/' ,YEAR(DFECHA)),CONCAT('01','/','0',MONTH(DFECHA), '/' ,YEAR(DFECHA))) AS FECHA,
SUM(ROUND((AICANTF*AIPRECIO)+(IF(IPORCIVA=0,(AICANTF*AIPRECIO)*((DPORCIVA/100)),0)),2)) AS MONTO
...
GROUP BY DALMACEN, MONTH(DFECHA), YEAR(DFECHA)

ORDER BY DALMACEN, DFECHA