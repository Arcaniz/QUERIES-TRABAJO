...
ROUND((AICANTF*AIPRECIO)+(IF(IPORCIVA=0,(AICANTF*AIPRECIO)*((DPORCIVA/100)),0)),2)AS PRECIO_TOTAL,
...
CATDESCR AS TIENDA,
DFECHA AS FECHA, 
IF(DALMACEN < 100, "BOUTIQUES", IF(DALMACEN <303,"MAYOREO" ,IF(DALMACEN <503,"FRANQUICIAS", "BODEGAS"))) AS CLASIFICA_ALMACEN,
IF(DALMACEN < 12, "BOUTIQUES", IF(DALMACEN <303,"MAYOREO" ,IF(DALMACEN <503,"FRANQUICIAS", IF(DALMACEN =909,"B2C",IF(DALMACEN =905,"B2B",IF(DALMACEN =919,"AMAZON",'VENTA BODEGAS')))))) AS 'Canal de venta',
SUBSTRING(ICOD,1,9)AS MODELO_PADRE,
FAM2.FAMDESCR AS 'NUEVO DEPTO',
FAM3.FAMDESCR AS 'NUEVO TIPO',
FAMA.FAMDESCR AS OFERTA,
IF(FAMA.FAMDESCR=0,'Rebaja','Linea') as 'Tabla de acción'
...
WHERE (DITIPMV='F' OR DITIPMV='T' OR DITIPMV='FL' OR DITIPMV='FX'OR   DITIPMV='TL'OR DITIPMV='NL'OR DITIPMV='TC')
 AND DFECHA>='2020-11-09' AND DFECHA<='2020-11-18'  
