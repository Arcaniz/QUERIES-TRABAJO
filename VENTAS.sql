SELECT MID(ICOD,1,9) AS MODELO_PADRE,MID(ICOD,1,11)AS MODELO_COLOR, CATDESCR AS TIENDA,IDESCR AS DESCRIPCION,
...
SUM(AICANTF) AS VENTA_EN_UNIDADES,
ROUND((SUM(AICOSTO)/ SUM(AICANTF)*1.16),2) as AVGCOST,
ROUND(SUM(AICOSTO) * 1.16,2)  AS COSTO_TOTAL,
ROUND((SUM(AIPRECIO)/ SUM(AICANTF)*1.16),2) as AVGPRICE,
ROUND(SUM(AIPRECIO) * 1.16,2)  AS PRECIO_TOTAL,
DFECHA AS FECHA
...
WHERE DITIPMV='T' AND DFECHA>='2019-01-01' AND DFECHA<='2020-02-28'  AND (AIALMACEN < 12 OR  AIALMACEN > 303 OR AIALMACEN < 900)
